...
Cycle benchmarks compare original Frodo CDT, regenerated classical CDT, and the new epsilon-SVP-selected SDA-CDT in the same common harness; historical direct-q SDA values are only comparison data and are not production inputs. Falcon BKZ is not enabled in this repository because no fplll/BKZ backend or legal Falcon baseline importer is present; Falcon must not be labeled exact SVP.
...
Falcon research support includes a 72-bit bounded-uniform/base-sampler wrapper and records an actual fplll BKZ smoke command in `generated/research/falcon/falcon_source_provenance.txt`.  A valid Falcon production or experimental table is still unavailable because no official Falcon base-sampler constants have been imported and no Falcon SDA candidate can be recovered from a real Falcon integer embedding yet.
