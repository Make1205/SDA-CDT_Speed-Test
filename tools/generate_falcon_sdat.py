...
This helper intentionally keeps Falcon outputs under generated/research/falcon.
...
def write_semantics(alpha, tail):
    ...
    with (OUT / "falcon_distribution_semantics.txt").open("w") as f:
        f.write("source_file=config/falcon.conf;src/sda_generation.c\n")
...
def main():
    ...
    (OUT/"falcon_source_provenance.txt").write_text(
        f"official_baseline_available=false\nsource_search=repository Falcon files plus current config/generation semantics\n"
        f"fplll_path={fpath}\nfplll_version={fver}\nfplll_help_file=generated/research/falcon/fplll_help.txt\n"
