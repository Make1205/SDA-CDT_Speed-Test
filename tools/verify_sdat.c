...
static const char *cfg_for(const char *p) {
    if (!strcmp(p, "frodo640")) return "config/frodo640.conf";
    if (!strcmp(p, "frodo976")) return "config/frodo976.conf";
    if (!strcmp(p, "frodo1344")) return "config/frodo1344.conf";
    if (!strcmp(p, "falcon")) return "config/falcon.conf";
    return 0;
}
...
int main(int argc, char **argv) {
    (void)argc;
    (void)argv;
    FILE *rep = fopen("generated/sda_verification_report.txt", "w");
    if (!rep) return 1;
