...
Relaxed Frodo-640 compact-q research now distinguishes `theorem-aligned-certified`, `exact-svp-derived-post-verified`, and `rejected` candidates. Relaxed candidates may lie outside the theorem interval, but they still must be exact-SVP-derived and pass explicit normalization/support/SD/RD checks; fixed-q rounding and target-q search remain excluded.
