...
OUT_BASE=${ONLINE_BENCH_OUTPUT_DIR:-$ROOT/generated/online/formal}
...
python3 "$ROOT/tools/summarize_online_benchmarks.py" --raw "$RAW" --out-dir "$OUT" $([ "$FORMAL" -eq 1 ] && echo --formal || true) $([ "$QUICK" -eq 1 ] && echo --quick || true)
