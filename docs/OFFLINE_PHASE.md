...
### Relaxed exact-SVP-derived post-verification mode

Relaxed Frodo-640 research may scan `0 < epsilon < 1` outside the theorem-aligned interval, but epsilon remains the only search input and q must still come from the certified exact-`l_infinity` shortest vector. Such candidates are classified separately as `exact-svp-derived-post-verified`: they are not claimed to satisfy the Proposition-9 sufficient compactness interval, and they are accepted only if post-verification of normalization, centered/contiguous support, SD, RD, and certificates succeeds. Continuous tail-zero truncation is allowed; interior holes are rejected. The final report records `candidate_class`, PMF hash, effective support, tail-zero count, final SD/RD, expected logical bits, and table-size metrics.
