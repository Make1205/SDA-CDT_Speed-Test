...
For relaxed Frodo-640 sweeps, run each epsilon subrange in an isolated temporary working directory and merge only the trace CSV into the report helper. The helper keeps different `(q, PMF hash, effective support)` records separate so equal denominators with different PMFs/support are not collapsed.
