...
Relaxed Frodo-640 frontier rows include effective-support and table-size fields derived from the actual PMF: `effective_support_max`, `tail_zero_count`, `packed_table_bits`, and `native_table_bytes`. These rows remain research-only until a promoted candidate updates the online manifest and table hash.
