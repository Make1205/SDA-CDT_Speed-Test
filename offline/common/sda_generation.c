...
int sda_validate_signed_candidate(const sda_config*cfg,const long long*p,size_t n,sda_u128 q){
  ...
  sda_u128 sum=0;
  for(size_t i=0;i<n;i++){ if(p[i]<0) return -3; if((sda_u128)p[i]>(sda_u128)-1-sum) return -4; sum+=(sda_u128)p[i]; }
  return sum==q?0:-5;
}
...
static int solve_svp_candidate(const sda_config*cfg,mpfr_t*a,size_t n,mpfr_t eps,sda_generation_result*out){ sda_exact_linf_sda_result svp; sda_exact_linf_sda_init(&svp,n,cfg->mpfr_precision); int rc=sda_exact_linf_sda_solve(a,n,eps,0,&svp); if(rc||!svp.global_svp_certified){ sda_exact_linf_sda_clear(&svp); return rc?rc:-5; }
  ...
  out->pmf_is_fixed_q_normalized=0; out->final_q_from_exact_svp=1; finalize_metrics(a,n,out,cfg->renyi_order); out->baseline_dominance_certified=baseline_ok(out) && acceptance_meets_historical(cfg,out->q); out->production_eligible=out->baseline_dominance_certified&&out->global_svp_certified; power_metrics(out); mpfr_div(out->candidate_sd_ratio,out->sd_infinite,out->baseline_sd_infinite,MPFR_RNDN); mpfr_div(out->candidate_renyi_ratio,out->renyi,out->baseline_renyi,MPFR_RNDN); sda_exact_linf_sda_clear(&svp); return out->production_eligible?0:-8; }
